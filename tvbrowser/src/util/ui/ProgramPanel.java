...
private static Font getDynamicFontSize(Font font, int offset) {
  ...
  public void setProgram(Program program, int maxHeight) {
    ...
    if (programChanged
      ...
      int height = calculateHeight(titleHeight, descHeight, additionalHeight);
      ...
      mPreferredHeight = calculateHeight(titleHeight, mDescriptionIcon.getHeightForLineCount(maxDescLines), additionalHeight);
  ...
  private int calculateHeight(int titleHeight, int descriptionHeight, int addtionalHeight) {
    return titleHeight + descriptionHeight + mPictureAreaIcon.getIconHeight() + addtionalHeight + V_GAP;
  }
    ...
    if (markedByPluginArr.length != 0 && program.getMarkPriorityMax() > Program.PRIORITY_MARK_NONE) {
      ...
      if(priorites != null && priorites.length > 0) {
        ...
        if(Settings.Markings.WITH_MARKINGS_SHOWING_BORDER.getBoolean()) {
          ...
          grp.drawLine(x, y+1, x, height);
          
          grp.setColor(r);
          grp.drawLine(width-1, y+1, width-1, height);
          grp.drawLine(x+1, height-1, width-2, height-1);
        }
        else {
