...
public class DegenderPlugin extends Plugin {
  ...
  private static final Pattern GENDERED = Pattern.compile("(\\b(?i)(die\\s){0,1}(?-i)\\b(\\w+?)(?:\\s*[\\*\\:](?i:i)|I)n(nen){0,1})", Pattern.DOTALL | Pattern.UNICODE_CHARACTER_CLASS); 
  private static final Pattern GENDERED_LONG = Pattern.compile("(\\b(\\w+?)innen\\b\\s+(?:und|oder)\\s+\\b(\\w+?)\\b)|(\\b(\\w+?)\\b\\s+(?:und|oder)\\s+\\b(\\w+?innen)\\b)", Pattern.DOTALL | Pattern.UNICODE_CHARACTER_CLASS);
  ...
  public PluginInfo getInfo() {
    return new PluginInfo(DegenderPlugin.class, "DeGender", "Entfernt Binnen-I, Gender-* und Gender-: aus Titel, Episodentitel, Serientitel, Kurzbeschreibung, Beschreibung, Bildbeschreibung und weitere Informationen.", "René Mach");
  }
