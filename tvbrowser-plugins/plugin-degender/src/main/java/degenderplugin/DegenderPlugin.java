    ...
    if(mRemoveLongForm && text != null) {
      ...
      while(m.find(pos)) {
        ...
        if(needle != null && male != null && female != null && male.startsWith(female)) {
          result.append(text.substring(pos,index)).append(replace.substring(0, 1).toUpperCase()).append(replace.substring(1));
          mCountLong++;
        }
        else {
  ...
  private String deGenderPartizip(String text, Program p) {
    if(mReplacePartizip && text != null) {
      ...
      while(m.find(pos)) {
        ...
        System.out.println(replace + " " + (m.group(3) == null || !m.group(3).trim().equals("die")));
