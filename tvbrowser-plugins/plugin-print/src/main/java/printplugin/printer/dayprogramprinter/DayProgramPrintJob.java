...
public class DayProgramPrintJob extends AbstractPrintJob {
  ...
  protected Page[] createPages(PageModel pageModel) {
    ...
    for (int i = 0; i < result.length; i++) {
      ...
      result[i] = new ChannelPage(cols, getPageFormat(), mSettings.getColumnCount(), mSettings.getChannelsPerColumn(),
          pageModel.getHeader(), pageModel.getFooter(), mSettings.getDayStartHour(), mSettings.getDayEndHour(),
          mSettings.getProgramIconSettings()/* PrinterProgramIconSettings.create() */);
    }
   ...
   */
  private static ColumnModel[] getColumns(PageModel model, int fromInx, int cnt) {
  ...
  private static class ChannelPage implements Page {
    ...
    public ChannelPage(ColumnModel[] cols, PageFormat pageFormat, int maxColsPerPage, int channelsPerColumn,
        String header, String footer, int startHour, int endHour, ProgramIconSettings settings) {

      int tableHeight = ((int) pageFormat.getImageableHeight() - HEADER_SPACE - FOOTER_SPACE
          - (channelsPerColumn - 1) * TABLE_SPACE) / channelsPerColumn;
      ...
      for (int i = 0; i < channelsPerColumn; i++) {
        ...
        mProgramTableIcons[i] = new ProgramTableIcon(columns, (int) pageFormat.getImageableWidth(), tableHeight,
            maxColsPerPage, startHour, endHour, settings);
      }
    ...
    public void printPage(Graphics graphics) {
      ...
      for (int i = 0; i < mProgramTableIcons.length; i++) {
        if (mProgramTableIcons[i] != null) {
          mProgramTableIcons[i].paintIcon(null, graphics, x0,
              y0 + HEADER_SPACE + (mProgramTableIcons[i].getIconHeight() + TABLE_SPACE) * i);
        }
      }
    }
  }
