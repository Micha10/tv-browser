...
public class ProgramTableIcon implements Icon {
  ...
  public ProgramTableIcon(ColumnModel[] cols, int width, int height, int maxColsPerPage, int startHour, int endHour,
  ...
  public void paintIcon(final Component component, final Graphics graphics, final int x, final int y) {
    ...
    for (int i = 0; i < mColHeaders.length; i++) {
      ...
      int cw = COLUMN_WIDTH;
      // g.setClip(COLUMN_WIDTH * i, -HEADER_SPACE, COLUMN_WIDTH, HEADER_SPACE);
      g.drawString(mColHeaders[i], cw * i + (cw - w) / 2,
          -(HEADER_SPACE - (int) metrics.getStringBounds(mColHeaders[i], g).getHeight() - metrics.getLeading()
              + metrics.getDescent() + 2) / 2);
      // g.setClip(null);
    }
    ...
    for (ProgramItem[] mProgramItem : mProgramItems) {
      for (ProgramItem element : mProgramItem) {
        element.paint(g);
      }
    }
